...
export default function WatchlistScreen() {
  ...
  const { watchlist, moverParaAssistidos, removerDaWatchlist } = useFilmes();
  ...
  }, [watchlist, busca, generoAtivo, ordem]);
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerStats}>
          <View style={styles.statChip}>
            <Ionicons name="bookmark-outline" size={13} color="#c9973a" />
            <Text style={styles.statChipText}>{watchlist.length} {watchlist.length === 1 ? 'filme' : 'filmes'}</Text>
          </View>
          <View style={styles.statChip}>
            <Ionicons name="star" size={13} color="#c9973a" />
            <Text style={styles.statChipText}>
              Média {watchlist.length > 0 ? (watchlist.reduce((acc, f) => acc + f.nota, 0) / watchlist.length).toFixed(1) : '—'}
            </Text>
          </View>
        </View>
      </View>
    ...
    <FlatList
      ...
      ListEmptyComponent={
        <View style={styles.vazia}>
          <Ionicons name={watchlist.length === 0 ? 'bookmark-outline' : 'search-outline'} size={52} color="#2e2b45" />
          <Text style={styles.vaziaTitle}>
            {watchlist.length === 0 ? 'Tudo assistido!' : 'Nenhum resultado.'}
          </Text>
          <Text style={styles.vaziaText}>
            {watchlist.length === 0 ? 'Adicione filmes para ver depois.' : 'Tente outro termo ou filtro.'}
          </Text>
        </View>
      }
      renderItem={({ item }) => (
        ...
        >
          ...
          <View style={styles.acoes}>
            <TouchableOpacity style={styles.checkBtn} onPress={() => moverParaAssistidos(item.id)}>
              <Ionicons name="checkmark-circle-outline" size={28} color="#2e2b45" />
            </TouchableOpacity>
            <TouchableOpacity style={styles.removeBtn} onPress={() => removerDaWatchlist(item.id)}>
...
const styles = StyleSheet.create({
  ...
  notaRow: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  notaText: { fontSize: 12, color: '#c9973a', fontWeight: '700' },
  ...
  vaziaText: { fontSize: 14, color: '#2e2b45', textAlign: 'center' },
});
